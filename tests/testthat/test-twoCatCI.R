...
test_that("Function outputs data sets with desired information",{
  occi <- twoCatCI(obs_data=PPA, imp_data_list=PPAm5, type="partially", vars=c("sex", "race"), sig=3)
...
test_that("Outputs correct number of significant digits", {
  siglen <- 3
  occi <- twoCatCI(obs_data=PPA, imp_data_list=PPAm5, type="partially", vars=c("sex", "race"), sig=siglen)
